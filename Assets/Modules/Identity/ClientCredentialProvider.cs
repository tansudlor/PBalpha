...
namespace com.playbux.identity
{
    ...
    public class UserLevelClearanceProvider
    {
        ...
        public bool HasClearance(int clearance)
        {
            if (!userWithClearances.ContainsKey(clearance))
                return false;

            const ushort minimumLevel = (ushort)UserClearanceLevel.Moderator;

            if (!clearances.ContainsKey(userWithClearances[clearance]))
                return false;

            return clearances[userWithClearances[clearance]].ClearanceLevel >= minimumLevel;
        }

        public string GetClearanceData(int clearance)
        {
            if (!userWithClearances.ContainsKey(clearance))
                return "";

            return !clearances.ContainsKey(userWithClearances[clearance]) ? "" : clearances[userWithClearances[clearance]].Title;
