...
namespace Mirror
{
    ...
    public class SpatialHashingInterestManagement : InterestManagement
    {
        ...
        public override void GetObserverAt(Vector3 position, HashSet<NetworkIdentity> identities)
        {
            Vector2Int projected = ProjectToGrid(position);
            identities.RemoveWhere(id => (projected - ProjectToGrid(id.transform.position)).sqrMagnitude > 2);
        }

        public override int[] GetPosition(Vector3 position)
        {
            Vector2Int projected = ProjectToGrid(position);
            List<int> neighbors = new List<int>();
            neighbors.Add(projected.y * resolution + projected.x);
            ...
            for (int dy = -6; dy <= 6; dy++)
            {
                for (int dx = -6; dx <= 6; dx++)
                {
                    ...
                    int nx = projected.x + dx;
                    int ny = projected.y + dy;
